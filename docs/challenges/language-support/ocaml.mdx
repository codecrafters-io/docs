...
<Steps>
  <Step title="Local Environment">
    You can add dependencies by running `opam install` and editing the `dune` file.

    For example, to add the `yojson` json library, first run the following command:

    ```bash
    opam install yojson
    ```

    Then update the `dune` file to include the dependency:
  ...
  <Step title="Remote Testing">
    The test runner needs to know which dependencies to install for your code.

    Before testing your code remotely, you should edit the `dune-project` file:
    ...
    Run this command to update `codecrafters_redis.opam`:

    ```
    dune build
    ```

    And remember to commit the modified files:
